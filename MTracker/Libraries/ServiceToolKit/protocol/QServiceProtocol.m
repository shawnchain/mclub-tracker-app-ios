...
#pragma mark - MServiceProtocol

NSString* const kMSERVICE_PROTOCOL_MTOP = @"MTOP";
NSString* const kMSERVICE_PROTOCOL_DEFAULT = @"JoyAPI";

@implementation QServiceProtocol

+(QServiceProtocol*)defaultProtocol{
    return [self protocolNamed:kMSERVICE_PROTOCOL_DEFAULT];
}

+(QServiceProtocol*)protocolNamed:(NSString*)protocolName{
    if(protocolName == nil || [kMSERVICE_PROTOCOL_DEFAULT isEqualToString:protocolName]){
        // the DEFAULT protocol
        return [[[QServiceProtocol_JOYAPI alloc] init] autorelease];
    }else if([kMSERVICE_PROTOCOL_MTOP isEqualToString:protocolName]){
        // the MTOP protocol
        return nil;
    }else{
        return nil;
